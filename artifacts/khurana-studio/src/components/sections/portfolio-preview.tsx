        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
          {isLoading ? (
            Array.from({ length: 6 }).map((_, i) => (
              <Skeleton key={i} className={`w-full rounded-none ${i % 3 === 0 ? 'aspect-square' : 'aspect-[3/4]'}`} />
            ))
          ) : (
              ...
              {filteredItems?.slice(0, 6).map((item, index) => (
                <motion.div
                  ...
                  className={`group relative overflow-hidden cursor-pointer ${
                    index === 0 || index === 3 ? "lg:col-span-2 aspect-[16/9] lg:aspect-[21/9]" : "aspect-[4/5]"
                  }`}
                ...
                >
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-10 flex flex-col items-center justify-center text-white">
                    <span className="font-serif text-2xl md:text-3xl mb-2 translate-y-4 group-hover:translate-y-0 transition-transform duration-500">{item.title}</span>
                    <span className="font-sans text-xs uppercase tracking-widest text-white/70 -translate-y-4 group-hover:translate-y-0 transition-transform duration-500">{item.category}</span>
                  </div>
                  <img 
                    src={item.imageUrl || `/images/portfolio-${index + 1}.jpg`} 
                    alt={item.title}
        ...
        {lightboxIndex !== null && filteredItems && (
          ...
          >
            ...
            <motion.img
              ...
              src={filteredItems[lightboxIndex].imageUrl || `/images/portfolio-${lightboxIndex + 1}.jpg`}
